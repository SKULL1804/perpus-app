      ...
      <div class="col-12">
        <div class="card mb-4">
          <div class="card-header d-flex align-items-center">
            <button class="btn btn-primary mb-0" data-bs-toggle="modal" data-bs-target="#Kelas"><i class="fa-solid fa-plus me-2"></i>Tambah</button>
            @include('backend.super-admin.kelas.modal-create')
          ...
          <div class="card-body px-0 pt-0 pb-2">
            <div class="table-responsive p-0">
              <table class="table align-items-center mb-0">
                ...
                <tbody>
                    @php
                    $ids = 1 + (($kelas->currentPage() - 1) * $kelas->perPage());
                    @endphp
                    @if (count($kelas))
                        @foreach ($kelas as $k )
                        <tr>
                          ...
                          <td>
                            <p class="text-xs font-weight-bold mb-0">{{ $k->name }}</p>
                          </td>
                          <td class="d-flex gap-2">
                            <button type="button" class="btn btn-success mb-0" data-bs-toggle="modal" data-bs-target="#kelas{{ $k->id }}">Edit</button>
                            @include('backend.super-admin.kelas.modal-edit')
            ...
            <div class="px-4 pt-2">
                {!! $kelas->appends(Request::except('page'))->render('vendor.pagination.bootstrap-5') !!}
            </div>
          </div>
        </div>
      </div>
